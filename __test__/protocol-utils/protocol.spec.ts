...
describe("Protocol Utils", () => {
  ...
  it("should check if URL is root", () => {
    ...
    expect(isRootUrl("localhost:3000/users", trimProtocol(BASE_DEV_URL))).toBe(false);
    expect(isRootUrl("localhost:3000?user=true", trimProtocol(BASE_DEV_URL))).toBe(false);
